...
def validate_char(char: str) -> None:
    ...
    Traceback (most recent call last):
       ...
    ValueError: Unexpected symbol: A

    >>> validate_char('!')
    Traceback (most recent call last):
       ...
    ValueError: Unexpected symbol: !

    >>> validate_char('3')
    Traceback (most recent call last):
       ...
    ValueError: Unexpected symbol: 3
    """
...
def validate_dictionary_word(word: str) -> None:
    ...
    :raises ValueError: If the word does not meet the length requirements.
    ...
    Traceback (most recent call last):
    ...
    ValueError: Word 'a' does not meet length requirements (2-20)

    >>> validate_dictionary_word("extraordinarilylongword")  # This should fail
    Traceback (most recent call last):
    ...
    ValueError: Word 'extraordinarilylongword' does not meet length requirements (2-20)
    """

    if not (MIN_DICTIONARY_LENGTH <= len(word) <= MAX_DICTIONARY_LENGTH):
        raise ValueError(f"Word '{word}' does not meet length requirements ({MIN_DICTIONARY_LENGTH}-{MAX_DICTIONARY_LENGTH})")
...
def validate_input_string(line_number: int, input_string: str) -> None:
    ...
    :raises ValueError: If the line number exceeds the input size limit or
    ...
    Traceback (most recent call last):
    ...
    ValueError: Input file exceeds 100 lines limit

    >>> validate_input_string(5, "")  # This should fail due to short input length
    Traceback (most recent call last):
    ...
    ValueError: Line 5 does not meet length requirements (2-500)
    ...
    if not (1 <= line_number <= MAX_INPUT_SIZE):
        raise ValueError(f"Input file exceeds {MAX_INPUT_SIZE} lines limit")

    if not (MIN_INPUT_LENGTH <= len(input_string.strip()) <= MAX_INPUT_LENGTH):
        raise ValueError(f"Line {line_number} does not meet length requirements ({MIN_INPUT_LENGTH}-{MAX_INPUT_LENGTH})")
