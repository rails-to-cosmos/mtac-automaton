...
def main(dictionary_path: str, input_path: str):
    ...
    with open(dictionary_path, 'r', encoding='utf-8') as dictionary_file:  # Read and add words from the dictionary
        for line in dictionary_file:
            word = line.strip()
            matcher.add_word(word)

    with open(input_path, 'r', encoding='utf-8') as input_file:  # Read the input file and scan for matches
